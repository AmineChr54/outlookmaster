...
const Sidebar: React.FC<SidebarProps> = ({ collapsed = false, mailbox, setMailbox }) => {
  return (
    <aside
      className={`bg-white border-r flex flex-col p-4 gap-4 transition-all duration-300 ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      <nav className="flex flex-col gap-2">
        {MAILBOXES.map((item) => (
          <button
            key={item.key}
            className={`flex items-center gap-3 px-3 py-2 rounded font-medium transition-all duration-300 cursor-pointer ${
              collapsed ? "justify-center px-0" : "text-left"
            } ${mailbox === item.key ? "bg-blue-100 text-blue-700" : "text-black hover:bg-blue-50"}`}
      ...
      <div className="mt-auto">
        <button
          className={`w-full cursor-pointer bg-blue-600 text-white py-2 rounded hover:bg-blue-700 font-semibold transition-all duration-300 ${
