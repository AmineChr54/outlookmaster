...
const MessageListItem: React.FC<MessageListItemProps> = ({ email, selected, onClick }) => (
  <li
    className={`group cursor-pointer px-6 py-4 transition-all duration-150 flex flex-col gap-1 hover:bg-blue-50 ${selected ? 'bg-blue-100' : ''}`}
    onClick={onClick}
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(email.from || 'User')}&background=random`} alt="avatar" className="w-7 h-7 rounded-full border" />
        <span className="font-semibold text-gray-900 group-hover:text-blue-700 truncate max-w-xs">{email.subject || "(no subject)"}</span>
      </div>
